...
You can select a range from 1 hour (minimum) to 2 weeks (maximum). The chart displays the total query load in intervals of 1 minute to 1 day, depending on the range you selected.
...
Queries that were actively running during the interval. Note that they may have started running before and continued running after the interval.
...
Queries that were waiting to run due to warehouse overload (i.e. waiting for other queries to finish running and free compute resources).
